...
def help(cmd, idx, payload, payload_json)
    ...
    mqttprint("save <0|1> : 1 enables minute wake mode (WiFi wakes every minute, ON 15s)")
    ...
    mqttprint("for guaranteed deferred execution while WiFi sleeps, publish retained command to app/<client>/<ville>/<device>/set/CMD")
    mqttprint("example payload: upgrade 1  or  update")
    mqttprint("use save 0 for continuous network")
    tasmota.resp_cmnd_done()
end
