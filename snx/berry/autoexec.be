...
def help(cmd, idx, payload, payload_json)
    ...
    mqttprint("stm32reset [out|in] : pulse reset pins for STM32 (default: out)")
    mqttprint("set <mode> : set/show publish mode string")
    mqttprint("snxready [0|1] : read or set READY pin level")
    mqttprint("snxdebug [0|1] : read or set debug logging flag")
    mqttprint("snxlegacy [0|1] : read or set legacy publish mode flag")
    mqttprint("snxsend <text> : send raw text over UART")
    mqttprint("snxstatus : show C++ runtime counters (queue/drops/published/flash)")
    mqttprint("snxcout : trigger native COUT config sync")
    mqttprint("snxupload <path> : flash C031 via UART upload from local file")
    mqttprint("snxflash <1|2|3> : flash STM32 target on bus 1/2/3")
    mqttprint("h7flash <path> : flash H7 via SWD from local file")
