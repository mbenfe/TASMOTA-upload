var version = "2026-05-22 partitioned flash flasher for STM32F412 PWX legacy"

import global

class stm32f412_flasher
  ...
  var image_last_addr
  var erase_first_sector
  var erase_last_sector
  ...
  def init()
    ...
    self.erase_first_sector = -1
    self.erase_last_sector = -1
  ...
  def flash(filename)
    ...
    self._cmd_erase_sectors_auto(self.erase_first_sector, self.erase_last_sector)
  ...
  def _check_post()
    ...
    self.erase_first_sector = self._sector_for_addr(self.image_first_addr)
    self.erase_last_sector = self._sector_for_addr(self.image_last_addr)
    ...
    print(format("FLH: erase sectors %i..%i", self.erase_first_sector, self.erase_last_sector))
  end

  def _sector_for_addr(addr)
    if addr < 0x08004000 return 0 end
    if addr < 0x08008000 return 1 end
    if addr < 0x0800C000 return 2 end
    if addr < 0x08010000 return 3 end
    if addr < 0x08020000 return 4 end
    if addr < 0x08040000 return 5 end
    if addr < 0x08060000 return 6 end
    if addr < 0x08080000 return 7 end
    if addr < 0x080A0000 return 8 end
    if addr < 0x080C0000 return 9 end
    if addr < 0x080E0000 return 10 end
    if addr < self.FLASH_END return 11 end
    raise "value_error", format("address out of sector map addr=0x%08X", addr)
  end
  ...
  def _cmd_extended_erase_sectors(first_sector, last_sector)
    if first_sector < 0 || last_sector < first_sector
      raise "value_error", format("invalid erase sector range first=%i last=%i", first_sector, last_sector)
    end

    var sector_count = last_sector - first_sector + 1
    var n = sector_count - 1
    ...
    for sector:first_sector..last_sector
      p.add((sector >> 8) & 0xFF)
      p.add(sector & 0xFF)
    end
    ...
    p.add(self._xor_checksum(p))
    global.serflash.write(p)
    self._expect_ack(30000, format("cmd_extended_erase_sectors first=%i last=%i", first_sector, last_sector))
  end

  def _cmd_erase_sectors_legacy(first_sector, last_sector)
    if first_sector < 0 || last_sector < first_sector || last_sector > 0xFF
      raise "value_error", format("invalid legacy erase sector range first=%i last=%i", first_sector, last_sector)
    end
    ...
    p.add(self._xor_checksum(p))
    global.serflash.write(p)
    self._expect_ack(30000, format("cmd_erase_sectors_legacy first=%i last=%i", first_sector, last_sector))
  end

  def _cmd_erase_sectors_auto(first_sector, last_sector)
    if self._supports_cmd(0x44)
      print("FLH: erase command=0x44 (extended erase sectors)")
      self._cmd_extended_erase_sectors(first_sector, last_sector)
      return
    end
    if self._supports_cmd(0x43)
      print("FLH: erase command=0x43 (legacy erase sectors)")
      self._cmd_erase_sectors_legacy(first_sector, last_sector)
      return
    end
