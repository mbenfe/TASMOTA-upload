var version = "2026-05-20 partitioned flash flasher for STM32C071"
...
class stm32c071_flasher
  ...
  static FLASH_PAGE_SIZE = 0x00000800
  static CORE_VERSION = "2026-05-20-c071-partitioned-v1"
  ...
  def flash(filename)
    ...
    self._cmd_erase_pages_auto(self.erase_first_page, self.erase_last_page)
  ...
  def _check_post()
    ...
    self.erase_first_page = (self.image_first_addr - self.FLASH_START) >> 11
    self.erase_last_page = (self.image_last_addr - self.FLASH_START) >> 11
  ...
  def _cmd_extended_erase_pages(first_page, last_page)
    ...
    p.add(self._xor_checksum(p))
    global.serflash.write(p)
    self._expect_ack(30000, format("cmd_extended_erase_pages first=%i last=%i", first_page, last_page))
  end

  def _cmd_erase_pages_legacy(first_page, last_page)
    if first_page < 0 || last_page < first_page || last_page > 0xFF
      raise "value_error", format("invalid legacy erase page range first=%i last=%i", first_page, last_page)
    end
    ...
    p.add(self._xor_checksum(p))
    global.serflash.write(p)
    self._expect_ack(30000, format("cmd_erase_pages_legacy first=%i last=%i", first_page, last_page))
  end

  def _cmd_erase_pages_auto(first_page, last_page)
    if self._supports_cmd(0x44)
      print("FLH: erase command=0x44 (extended erase pages)")
      self._cmd_extended_erase_pages(first_page, last_page)
      return
    end
    if self._supports_cmd(0x43)
      print("FLH: erase command=0x43 (legacy erase pages)")
      self._cmd_erase_pages_legacy(first_page, last_page)
      return
    end
