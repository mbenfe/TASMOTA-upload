var version = "2026-05-18 bootloader-only H743 flasher (sector 0 only)"

import global

class stm32h743_boot_flasher
  static FLASH_START = 0x08000000
  static FLASH_END = 0x08200000
  static FLASH_PAGE_SIZE = 0x00020000
  static BOOT_FLASH_END = 0x08020000
  static BOOT_FLASH_PAGE = 0
  static CORE_VERSION = "2026-05-18-boot-sector-v1"
  ...
  def init()
    ...
    print("bootflasher hardware setup completed")
   self.file_hex = nil
  ...
  def flash(filename)
    ...
    print("BOOTFLH: flashing started")
    self._start_link()
    var idb = self._cmd_get_id()
    print("BOOTFLH: chip id bytes=" + str(idb))
    ...
    print("BOOTFLH: flashing completed")
  end

  def _check_pre()
    print("BOOTFLH: checking HEX file")
    print("BOOTFLH: core version " + self.CORE_VERSION)
    self.saw_flash_data = false
    self.check_records = 0
    self.check_bytes = 0
    self.image_first_addr = -1
  ...
  def _check_cb(addr, sz, data, offset)
    if addr < self.FLASH_START || addr + sz > self.BOOT_FLASH_END
      raise "value_error", format("bootloader address out of first-sector range addr=0x%08X len=%i", addr, sz)
    end
  ...
  def _check_post()
    ...
    if self.image_first_addr != self.FLASH_START
      raise "value_error", format("bootloader must start at 0x%08X, got 0x%08X", self.FLASH_START, self.image_first_addr)
    end

    self.erase_first_page = self.BOOT_FLASH_PAGE
    self.erase_last_page = self.BOOT_FLASH_PAGE

    print(format("BOOTFLH: HEX check OK records=%i bytes=%i", self.check_records, self.check_bytes))
    print(format("BOOTFLH: image span 0x%08X..0x%08X", self.image_first_addr, self.image_last_addr))
    print(format("BOOTFLH: erase page %i only (%i-byte page)", self.erase_first_page, self.FLASH_PAGE_SIZE))
  end
  ...
  def _flash_pre()
    ...
    self.flash_bytes = 0
    self.flash_writes = 0
    print("BOOTFLH: step 6/7 - writing aligned 32-byte flash words")
  end
  ...
  def _flash_post()
    self._flush_pending_slot()
    print(format("BOOTFLH: write summary records=%i bytes=%i aligned_writes=%i", self.flash_records, self.flash_bytes, self.flash_writes))
    print("BOOTFLH: step 6.5/7 - jump to bootloader")
    try
      self._cmd_go(self.image_first_addr)
    except .. as e, m
      print("BOOTFLH: warning: GO handshake non-fatal: " + m)
    end
    print("BOOTFLH: step 6.9/7 - restore normal boot mode")
    self._leave_system_bootloader()
  end

end

return stm32h743_boot_flasher()