...
def get_cron_second()
    var combined = string.format("%s|%s", global.ville, global.esp_device)
    var sum = 0
...
class AEROTHERME
    ...
    def heartbeat()
        ...
        var payload = string.format('{"Device":"%s","Name":"%s","Time":"%s","AccessPoint":"%s","IpAddress":"%s"}', global.esp_device, global.esp_device, timestamp, ap, ip)
        mqtt.publish(topic, payload, true)
    end
end
...
print("Cron second for device " + global.esp_device + " is " + str(cron_second))
...
tasmota.add_cron(cron_pattern, /-> aerotherme.heartbeat(), "every_hour_@0_s")